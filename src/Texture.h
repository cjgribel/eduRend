...
struct Texture
{
	...
	ID3D11ShaderResourceView* texture_SRV = nullptr; //!< Shader Resource View pointing to the GPU texture.
 ...
 * @detials Calls LoadTextureFromFile(ID3D11Device*,ID3D11DeviceContext*,const char*,Texture*) for the actual work.
 ...
 * @brief Loads a 2D texture from file.
 * @param[in] dxdevice Valid ID3D11Device device.
 * @param[in, opt] dxdevice_context If provided the ID3D11DeviceContext will be used to auto generate mip maps for the texture.
 * @param[in] filename File path to a valid image.
 * @param[out] texture_out Texture struct to store the resulting texture in.
 * @return HRESULT of the texture creation.
*/
